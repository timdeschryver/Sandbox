...
export class FormFieldValidationMessage implements PipeTransform {
	private readonly messages = {
		...defaultValidationMessages,
		...inject(VALIDATION_MESSAGES, { optional: true }),
	};

	public transform(error: ValidationError.WithOptionalFieldTree | null | undefined, showFieldName = false): string {
		if (!error) {
			return '';
		}
